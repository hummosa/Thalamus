...
# my_parser.add_argument('--experiment_type', default='rehearsal', nargs='?', type=str, help='Which experimental or setup to run: "pairs") task-pairs a b a "serial") Serial neurogym "interleave") Interleaved ')
...
my_parser.add_argument('--var3',  default=0, nargs='?', type=int, help='learn gates')
my_parser.add_argument('--var4', default='policy', nargs='?', type=str,  help='optim')
...
config.exp_signature = f"{args.var1}_{args.var3}_{args.var4}_"
config.num_of_tasks = args.num_of_tasks

config.train_gates = bool(args.var3)

config.optimize_policy  = True if str(args.var4) == 'policy' else False
config.optimize_td      = True if str(args.var4) == 'td' else False
config.optimize_bu      = True if str(args.var4) == 'bu' else False

config.saved_model_sig = f'seed{args.seed}_paradigm_{"shuf" if config.paradigm_shuffle else "seq"}_{"mul" if config.use_multiplicative_gates else "add"}_tasks_{config.num_of_tasks}_learn{config.train_gates}_optim_td'
...
config.gates_std = 0.01
config.gates_sparsity = 0.5#args.var4
...
if config.higher_order:
    ...
    config.random_rehearsals = 20 if config.paradigm_sequential else 20*12*config.num_of_tasks #sequential random reh sends 12 trials on avg per task. 
    config.save_model = False
    config.load_saved_rnn1 = not config.save_model
    config.test_task_variance = False
###--------------------------Training configs--------------------------###
...
if config.load_saved_rnn1:
    ...
    if config.test_task_variance:
        viz.plot_task_variance(config, net)
else:
...
if config.higher_order:
    ...
    optimize_tasks = [config.tasks_id_name[i] for i in range(len(config.tasks_id_name))]
    optimize_tasks = task_seq_sequential # use these for novel task upfront.
    optimize_tasks = task_seq_random[:50] # use these for novel task upfront.
    testing_log, training_log, net = optimize(config, net, cog_net, optimize_tasks, testing_log, training_log, step_i = step_i )
    print(f'the last {45} episodes average ttc is : {mean(training_log.trials_to_crit[-45:])}')
